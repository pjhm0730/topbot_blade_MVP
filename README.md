...
탑바텀블레이드는 2~10명이 짧게 즐기고, 마지막까지 살아남은 사람을 "오늘의 음료수 담당"으로 정하는 웹게임 MVP입니다.
...
8. Result screen에서 마지막 생존자와 각 플레이어의 선택 순번을 표시합니다.
...
- Battle 화면은 compact HUD, arena canvas, 오른쪽 실시간 생존 현황판으로 나뉘어 표시됩니다.
...
- Battle 화면에서는 현재 살아 있는 팽이 중 energy 비율이 가장 높은 마지막 생존 후보에 `생존 후보` 링과 배지가 표시됩니다.
- 최종 음료수 담당이 확정되면 해당 팽이에 더 강한 링, shockwave, `음료수 담당!` 라벨이 표시됩니다.
- 내 팽이가 마지막 생존 후보이거나 최종 음료수 담당인 경우에도 `내 팽이` 하이라이트가 함께 보입니다.
- 오른쪽 생존 현황판에는 모든 플레이어의 선택 순번, 닉네임, 스킨 미리보기, energy bar, 생존/탈락/생존 후보 상태가 실시간으로 표시됩니다.
- Result 화면에서는 음료수 담당 팽이를 크게 보여주고, 순위 목록에도 각 플레이어의 팽이 스킨 미리보기를 표시합니다.
- Result 화면에서 음료수 담당은 `담당`, 현재 테스트 플레이어는 `내 결과` 배지로 구분됩니다.
...
- 전투 초반 약 6.5초 동안은 충돌이 많아도 energy가 너무 빨리 0이 되지 않도록 보호됩니다.
- 보호 시간 이전에는 일반 충돌 데미지로 실제 탈락이 발생하지 않습니다.
- 보호 시간 이후부터 energy가 0이 된 팽이는 탈락 처리되지만, 게임은 바로 끝나지 않습니다.
- 살아 있는 팽이가 1명만 남으면 그 마지막 생존자가 오늘의 음료수 담당입니다.
- 목표 게임 길이는 대략 15~20초입니다.
- 최대 전투 시간은 20초이며, 20초가 지나면 아직 살아 있는 팽이 중 남은 energy가 가장 높은 플레이어가 음료수 담당입니다.
...
- `preEliminationEnergyFloorRatio`: 초반 보호 시간 전 energy 최저 보호 비율
- `collisionEnergyLossMultiplier`: 충돌 energy 감소량
- `passiveDrainMultiplier`: 시간 경과 energy 감소량
- `earlyPhaseDamageMultiplier`: 0~5초 충돌 데미지
- `midPhaseDamageMultiplier`: 5초부터 보호 시간 종료 전까지의 충돌 데미지
- `latePhaseDamageMultiplier`: 보호 시간 이후 충돌 데미지
